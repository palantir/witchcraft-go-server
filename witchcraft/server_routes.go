...
func (s *Server[I, R]) addRoutes(ctx context.Context, mgmtRouterWithContextPath wrouter.Router, runtimeCfg refreshable.Refreshable[R]) error {
	diagnosticRefreshable := refreshable.MapContext(ctx, runtimeCfg, func(cfg R) config.DiagnosticsConfig {
...
func (s *Server[I, R]) addMiddleware(rootRouter wrouter.RootRouter, registry metrics.RootRegistry, endpoint500s *middleware.EndpointFiveHundredsHealthCheck, tracerOptions []wtracing.TracerOption) {
...
func getSecretRefreshable(ctx context.Context, diagnosticsConfig refreshable.Refreshable[config.DiagnosticsConfig]) (refreshable.Refreshable[string], error) {
	...
	if secretFromConfig != "" {
		r := refreshable.MapContext(ctx, diagnosticsConfig, func(cfg config.DiagnosticsConfig) string {
			return cfg.DebugSharedSecret
		})
		return r, nil
	}
	fileRefreshable := refreshable.NewFileRefreshable(ctx, secretFilePath)
