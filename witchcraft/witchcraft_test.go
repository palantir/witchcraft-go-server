...
func TestServer_Start_WithStop(t *testing.T) {
	...
	for name, stop := range map[string]func(*witchcraft.Server[config.Install, config.Runtime], context.Context) error{
		...
		"multiple stop": func(server *witchcraft.Server[config.Install, config.Runtime], ctx context.Context) error {
		...
		"shutdown after ctx done": func(server *witchcraft.Server[config.Install, config.Runtime], ctx context.Context) error {
...
func testStop(t *testing.T, server *witchcraft.Server[config.Install, config.Runtime], stop func(*witchcraft.Server[config.Install, config.Runtime], context.Context) error) {
	// start server
	errc := make(chan error)
	go func() {
		errc <- server.Start()
